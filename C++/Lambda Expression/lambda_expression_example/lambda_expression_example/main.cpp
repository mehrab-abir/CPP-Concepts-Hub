...
void getDiscountAmount(int age) {
	...
	//'mutable' enables modifying captured variable, but modifies only inside the lambda function, not the original minAge variable.
	//to change the state of the original variable by lambda, capture it by reference
	...
	cout << "\n\nminAge after calling halfCharge() : " << minAge <<"\t - unchanged\n"<< endl;
}
